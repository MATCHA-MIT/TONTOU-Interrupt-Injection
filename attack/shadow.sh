...
gcc -pthread -no-pie -DSYSCALL=1 -DCORES=$CORES find_shadow.c -o find_shadow
...
tmux send-keys -t "$SESSION:0.0" "{ time ./find_shadow $_text $physmap ; } 2>&1 | tee logs/shadow.log &" C-m

# Create panes for logs
for ((i=0; i < CORES; i++)); do
  if [ "$i" -ne 0 ]; then
